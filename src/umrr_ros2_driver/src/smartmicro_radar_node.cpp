...
std::chrono::time_point<std::chrono::steady_clock> last_point_cloud_received;
...
: rclcpp::Node{"smartmicro_radar_node", node_options}
{
    ...
    if (
      ...
      timer_ = this->create_wall_timer(
        ...
        [this]() -> void {
            ...
            auto duration_since_last_data = std::chrono::duration_cast<std::chrono::seconds>(now - last_point_cloud_received).count();
            
            // Print the duration since the last point cloud data
            //RCLCPP_INFO(this->get_logger(), "Duration since last point cloud data: %d seconds", duration_since_last_data);

            if (duration_since_last_data > 10) { // Example threshold of 10 seconds
                RCLCPP_WARN(this->get_logger(), "No point cloud data received for %d seconds. Radar might be disconnected.", duration_since_last_data);
            }
        }
    );
  ...
  auto timer_callback = [this]() -> void {
      ...
      if (duration_since_last_data > 10 && duration_since_last_data < 60) { // Example threshold of 10 seconds
          RCLCPP_WARN(this->get_logger(), "No point cloud data received for %d seconds. Radar might be disconnected.", duration_since_last_data);
      }
  };

  // Ensure `timer_` is a class member, initialized in the constructor
  ...
  const com::types::ClientId client_id)
{
  ...
  if (!check_signal) {
    ...
    RCLCPP_INFO(this->get_logger(), "Point cloud data received from sensor", sensor_idx);
    sensor_msgs::msg::PointCloud2 msg;
    RadarCloudModifier modifier{msg, m_sensors[sensor_idx].frame_id};
    const auto timestamp = std::chrono::microseconds{port_header->GetTimestamp()};
    const auto sec = std::chrono::duration_cast<std::chrono::seconds>(timestamp);
    ...
    msg.header.stamp.sec = sec.count();
    msg.header.stamp.nanosec = nanosec.count();
    last_point_cloud_received = std::chrono::steady_clock::now();
    RCLCPP_INFO(this->get_logger(), "Updated last_point_cloud_received");
