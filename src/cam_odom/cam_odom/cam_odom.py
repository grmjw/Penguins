...
class ImageSubscriber(Node):
    ...
    def __init__(self):
        ...
        logger.debug('creating the subscriber')
        self.subscription = self.create_subscription(
            Image, 'video_frames', self.listener_callback, 100)
        self.subscription  # prevent unused variable warning

        # Used to convert between ROS and OpenCV images
        logger.debug('create CV bridge object')
        self.br = CvBridge()
        
        logger.debug('Opened camera calibration file')
        ...
        self.start_pose = np.concatenate(
            (self.start_rotation, self.start_translation), axis=1)
    ...
    def listener_callback(self, data):
        ...
        logger.debug('Converting ROS Image message to OpenCV image')
    ...
    def start_calc(self):
        ...
        if self.process_frames:
            ...
            logger.debug('updating estimated path')
            self.estimated_path.append((self.cur_pose[0, 3], self.cur_pose[2, 3]))

            estimated_camera_pose_x, estimated_camera_pose_y = self.cur_pose[0,
                3], self.cur_pose[2, 3]
        ...
        cv2.putText(self.new_frame, f'FPS: {int(fps)}', (20, 50),
                    cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)

        cv2.putText(self.new_frame, str(np.round(
            self.cur_pose[0, 0], 2)), (260, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 1)
        cv2.putText(self.new_frame, str(np.round(
            self.cur_pose[0, 1], 2)), (340, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 1)
        cv2.putText(self.new_frame, str(np.round(
            self.cur_pose[0, 2], 2)), (420, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 1)
        cv2.putText(self.new_frame, str(np.round(
            self.cur_pose[1, 0], 2)), (260, 90), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 1)
        cv2.putText(self.new_frame, str(np.round(
            self.cur_pose[1, 1], 2)), (340, 90), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 1)
        cv2.putText(self.new_frame, str(np.round(
            self.cur_pose[1, 2], 2)), (420, 90), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 1)
        cv2.putText(self.new_frame, str(np.round(
            self.cur_pose[2, 0], 2)), (260, 130), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 1)
        cv2.putText(self.new_frame, str(np.round(
            self.cur_pose[2, 1], 2)), (340, 130), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 1)
        cv2.putText(self.new_frame, str(np.round(
            self.cur_pose[2, 2], 2)), (420, 130), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 1)

        cv2.putText(self.new_frame, str(np.round(
            self.cur_pose[0, 3], 2)), (540, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 1)
        cv2.putText(self.new_frame, str(np.round(
            self.cur_pose[1, 3], 2)), (540, 90), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 1)
        cv2.putText(self.new_frame, str(np.round(
            self.cur_pose[2, 3], 2)), (540, 130), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 1)
        logger.debug('Displaying frame %d', self.frame_counter)
...
class OdomPublisher(Node):
    ...
    def publish_odom(self, x, y, z, quat_x, quat_y, quat_z, quat_w):
        ...
        logger.info('publishing odometry date')
...
def main(args=None):
    ...
    logger.info('Destroying image subscriber node')
