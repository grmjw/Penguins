  ...
  slam_toolbox__srv__AddSubmap_Request__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    slam_toolbox__srv__AddSubmap_Request * data =
      (slam_toolbox__srv__AddSubmap_Request *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!slam_toolbox__srv__AddSubmap_Request__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          slam_toolbox__srv__AddSubmap_Request__fini(&data[i]);
        }
        free(data);
        return false;
      }
  ...
  slam_toolbox__srv__AddSubmap_Response__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    slam_toolbox__srv__AddSubmap_Response * data =
      (slam_toolbox__srv__AddSubmap_Response *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!slam_toolbox__srv__AddSubmap_Response__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          slam_toolbox__srv__AddSubmap_Response__fini(&data[i]);
        }
        free(data);
        return false;
      }
